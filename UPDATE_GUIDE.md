...
End users of `foro` — developers who run foro to format TOML files. They are not plugin maintainers. They do not care about CI, dll-pack, or packaging internals.

### What to write

- State clearly which upstream `tombi` version is bundled.
- Summarize any notable changes from the `foro-tombi` branch that affect formatting output (new formatting rules, changed behavior, bugfixes that affect output).
- If this is a plugin-only fix (no upstream change), describe what was fixed in plain terms.
- Since tombi does not publish formal releases, infer changes from the git log on the `foro-tombi` branch between the previous and current pinned revisions.
...
Source: https://github.com/nahco314/tombi/tree/foro-tombi
...
```markdown
Bundles tombi **X.Y.Z**. No formatting behavior changes.
...
```bash
# Compare git log between previous and current rev on foro-tombi branch
git log --oneline <prev_rev>..<new_rev> -- https://github.com/nahco314/tombi
# Or clone and inspect:
git clone --depth=50 -b foro-tombi https://github.com/nahco314/tombi /tmp/tombi-upstream
git -C /tmp/tombi-upstream log --oneline | head -20
```
